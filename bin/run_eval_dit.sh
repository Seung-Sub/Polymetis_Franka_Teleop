...
#   <task> = peg | pap
#   <type> = fp32 | ptq | agiledp
# Examples:
#   bash bin/run_eval_dit.sh peg fp32 --max_duration 60
#   bash bin/run_eval_dit.sh pap ptq
#   bash bin/run_eval_dit.sh peg agiledp
set -u
...
echo "[run_eval_dit] task=$TASK type=$TYPE FP=$FP ${QUANT_CKPT:+quant=$QUANT_CKPT qtype=$QTYPE}"
