...
# Optional quantized / distilled deploy. Set these env vars to swap a quantized
# UNet into the FP teacher ($CKPT stays the FP ckpt). FP / agile-DP deploy needs
# none of them (QUANT_ARGS stays empty -> identical to before).
#   QUANT_CKPT=.../quantw8a8_naiveq.pth   QTYPE=naiveq|qalora|qalora_tada
#   N_BIT=8                               CALIB_DATA=.../DiffusionInput_calib.pth
QUANT_ARGS=""
if [ -n "${QUANT_CKPT:-}" ]; then
    : "${CALIB_DATA:?CALIB_DATA is required when QUANT_CKPT is set}"
    QUANT_ARGS="--quant_ckpt ${QUANT_CKPT} --qtype ${QTYPE:-naiveq} --n_bit ${N_BIT:-8} --calib_data ${CALIB_DATA}"
    echo "[wrapper] quant deploy enabled: ${QUANT_ARGS}"
fi
PYTHONPATH="$DAEMON_PYPATH" CONDA_PREFIX="/home/kist/anaconda3/envs/umi" PYTHONUNBUFFERED=1 \
    nohup "$UMI_PY" -u scripts_real/eval_dp_inference_daemon.py \
        --ckpt "$CKPT" --port "$DAEMON_PORT" --device cuda:0 $QUANT_ARGS \
        > "$DAEMON_LOG" 2>&1 &
disown
DAEMON_PID=$!
...
setsid python -u scripts_real/eval_dp_real.py \
