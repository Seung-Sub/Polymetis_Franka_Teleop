...
# Degenerate-chunk guard (2026-06-04): the identity-normalizer pap/peg ckpts run
# the DDIM sampler with clip_sample=False, so an OOD observation can yield an
# out-of-range action chunk (seen live: grip=1.16 outside [0,1], 183mm-mean /
# 324mm-max CONSECUTIVE steps vs the policy's normal <=~45mm). A chunk whose
...
class InferenceClient:
    ...
    def predict(self, obs_np: dict):
        ...
        # FIX A: prefer the full predicted horizon when the daemon ships it.
        # Returns (action_array, action_offset, inference_ms) where action_offset
        # is the number of leading steps that land at/before the last obs time
        # t_obs (so the caller times step k at t_obs + (k - action_offset)*dt).
        # For action_pred, offset = n_obs_steps-1 (=1). Falls back to the short
        # 'action' (offset 0; action[0] == t_obs) for an older daemon.
...
def main(daemon_port, robot_ip, steps_per_inference, max_duration, frequency,
    ...
    with SharedMemoryManager() as shm:
        ...
        try:
            while time.time() - eval_t_start < max_duration:
                ...
                # so the in-place gripper clamp below would raise ValueError.
                action_seq = np.array(action_seq[0], dtype=np.float32)   # (horizon, 7)
                # --- Action sanity / degenerate-chunk guard (see DEGENERATE_INTRA_STEP_M) ---
                # (1) gripper is physically in [0,1]; clamp unconditionally (the
                #     clip_sample=False sampler can emit e.g. 1.16).
                action_seq[:, 6] = np.clip(action_seq[:, 6], 0.0, 1.0)
                # (2) flag a chunk whose RAW consecutive position steps are far
                #     beyond the policy's normal smoothness -> OOD diffusion sample.
                ...
                # (c) UMI action timestamps.
                # FIX A: action_seq[k] is the action at t_obs + (k - action_offset)*dt.
                # With the full action_pred (action_offset = n_obs_steps-1 = 1) the
                # first `action_offset` steps land at/just before t_obs and are
                # pruned by the is_new filter below; the rest give a ~1.4 s future
                # horizon so this chunk overlaps the next inference's chunk and the
                # arm never stalls at a chunk boundary (was a 200ms hold-and-jump).
                # action_offset = 0 reproduces the old single-chunk behaviour.
                ...
                # Degenerate chunk -> do NOT execute it. Hold the current pose for
                # one dt so the cartesian-impedance controller keeps getting a
                # command (and is not auto-recovered) while we wait for the next,
                # hopefully in-distribution, inference. Takes precedence over the
                # over-budget branch above (holding is always the safe choice).
                ...
                # each cycle. FIX A schedules a ~1.4 s horizon, so the PREVIOUS
                # cycle's last applied waypoint is a far-future pose; carrying it
                # into this cycle's first kept step (only ~0.1 s ahead) manufactured
                # a spurious 200-400 mm "step" -> false STEP-reject cascade -> the
                # arm held, then resumed discontinuously and tripped a libfranka
                # reflex ("no controller running" -> recovery jerk). The RAW policy
                # chunk is smooth (consecutive steps <=~45 mm, verified via the
                # daemon diag), so seeding from the actual TCP makes the guard
                # check intra-chunk smoothness, which is its real intent.
            ...
            try:
                ...
                if hasattr(ctrl, 'move_home'):
                    # FrankaInterpolationController.move_home() takes NO args
                    # (duration is the controller's self.home_time). Passing
                    # time_to_go=4.0 raised "unexpected keyword argument
                    # 'time_to_go'" so the arm never returned home at shutdown
                    # (observed live 2026-06-04). Call it bare.
                    ctrl.move_home()
